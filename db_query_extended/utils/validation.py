...
SUPPORTED_DATABASE_TYPES = {"mysql", "postgresql", "dm"}
DEFAULT_PORTS = {"mysql": 3306, "postgresql": 5432, "dm": 5236}
...
def validate_connection_config(credentials: dict[str, Any]) -> dict[str, Any]:
    ...
    if database_type not in SUPPORTED_DATABASE_TYPES:
        raise UnsupportedDatabaseTypeError(
            "Unsupported database type. Only mysql, postgresql, and dm are available in this phase."
        )
    ...
    normalized["schema"] = str(
        credentials.get("schema") or (credentials.get("database") if database_type == "dm" else "") or ""
    ).strip()
